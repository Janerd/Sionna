...
class SimConfig:
    ...
    # 小区数量
    # umi 场景：7 个小区（1 中心 + 6 邻区），ISD=200m
    # uma 场景：19 个小区（1 中心 + 两圈邻区），ISD=500m
    num_cells: int = 7

    # 站间距 [m]
    # umi: 200m（密集城区，小区边界密集，A3 切换频繁）
    ...
    # 3种速度 × 20条 = 60条（与 MATLAB smooth 场景相同）
    # 如果需要更多数据，可以增加到 120 条
    ...
    #   3. 在此处填入坐标
    #
    # 示例（7 小区，ISD≈200m，基于慕尼黑街道布局）：
    # bs_positions_override = np.array([
    #     [  0.0,   0.0],  # BS0: 场景中心（主要十字路口）
    #     [200.0,   0.0],  # BS1: 东侧街道
    #     [100.0, 173.0],  # BS2: 东北街道
    #     [-100.0, 173.0], # BS3: 西北街道
    #     [-200.0,  0.0],  # BS4: 西侧街道
    #     [-100.0,-173.0], # BS5: 西南街道
    #     [100.0, -173.0], # BS6: 东南街道
    # ], dtype=np.float32)
    #
